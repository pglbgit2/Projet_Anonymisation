...
from pyspark.sql.functions import avg, hour, dayofweek, randn, col, weekofyear, monotonically_increasing_id, expr, row_number
...
def anonymiseDayNightWeekend(startOfTheDay, startOfTheNight, df, variation, path):
    ...
    dfWeekend = df.filter(((dayofweek(df["timestamp"])) == 1 | (dayofweek(df["timestamp"])) == 7) & ((hour(df["timestamp"]) >= startOfTheDay) & (hour(df["timestamp"]) < startOfTheNight)))
    ...
    values_jour = dfDay.join(avg_jour, ["id", "week"]).withColumn("new_longitude", col("avg_longitude") + randn(
        seed=23) * variation).withColumn("new_latitude", col("avg_latitude") + randn(seed=42) * variation).drop(
        "avg_longitude", "avg_latitude")
    values_nuit = dfNight.join(avg_nuit, ["id", "week"]).withColumn("new_longitude", col("avg_longitude") + randn(
        seed=34) * variation).withColumn("new_latitude", col("avg_latitude") + randn(seed=42) * variation).drop(
        "avg_longitude", "avg_latitude")
    values_weekend = dfWeekend.join(avg_weekend, ["id","week"]).withColumn("new_longitude", col("avg_longitude") + randn(
        seed=26) * variation).withColumn("new_latitude", col("avg_latitude") + randn(seed=42) * variation).drop(
        "avg_longitude", "avg_latitude")
    ...
    # CSVManager.writeTabCSVFile(ready_to_be_anonymised.toPandas(), path)
    final = ready_to_be_anonymised.withColumn("timestamp",
